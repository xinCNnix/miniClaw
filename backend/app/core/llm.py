...
def _build_llm(llm_config: LLMConfig) -> BaseChatModel:
    ...
    # 非 LLM 服务类型（asr/tts/ocr）和 ollama 不强制要求 API Key
    needs_api_key = (
        llm_config.service_type == "llm"
        and llm_config.provider != "ollama"
    )
    if not llm_config.api_key and needs_api_key:
        raise ValueError(f"API key not configured for LLM '{llm_config.id}'")

    if not llm_config.api_key and not needs_api_key:
        logger.warning(
            f"No API key for {llm_config.id} (service_type={llm_config.service_type}, "
            f"provider={llm_config.provider}). Proceeding — will fail if service requires auth."
        )
