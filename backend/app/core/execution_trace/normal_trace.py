"""
Normal Mode Execution Trace

Replaces AgentExecutionLogger with NormalTrace(BaseExecutionTrace).
Preserves dual-mode API: manual mode (chat.py event stream) and
callback mode (LangChain TrajectoryCallbackHandler).
"""
...
class NormalTrace(BaseExecutionTrace):
    """Normal mode execution trace with dual-mode trajectory recording.
    ...
    def __enter__(self) -> "NormalTrace":
        self._start_dt = datetime.now()
        self.start_time = self._start_dt.timestamp()
        self.logger.info("=" * 80)
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        self.end_time = self._end_dt.timestamp()
    ...
    def log_step(self, thought: str, action: str, input_data: dict) -> int:
        """Record a step via callback mode. Returns the step number assigned."""
        if not self._enable_trajectory:
            return -1
    ...
    def get_trajectory(self) -> dict:
        ...
        if self._start_dt and self._end_dt:
            total_duration = (self._end_dt - self._start_dt).total_seconds()
        elif self._start_dt:
            total_duration = (datetime.now() - self._start_dt).total_seconds()
        ...
        return {
            ...
            "user_question": self.user_question,
            "start_time": self._start_dt.isoformat() if self._start_dt else None,
            "end_time": self._end_dt.isoformat() if self._end_dt else None,
    ...
    def get_summary(self) -> dict:
        """Return execution summary (alias for get_trajectory for BaseExecutionTrace compat)."""
    ...
    def save_trajectory(self) -> str | None:
        ...
        return save_trace(data, mode="normal", task_name=self.task_name, session_id=self.session_id or "")
