...
Display HTML content on connected miniClaw nodes (Mac app, iOS, Android).

## Overview

The canvas tool lets you present web content on any connected miniClaw node's canvas view. Great for:
...
```
http://<tailscale-hostname>:18793/__miniclaw__/canvas/<file>.html
```
...
```bash
cat ~/.miniclaw/miniclaw.json | jq '.gateway.bind'
```

Then construct the URL:

- **loopback**: `http://127.0.0.1:18793/__miniclaw__/canvas/<file>.html`
- **lan/tailnet/auto**: `http://<hostname>:18793/__miniclaw__/canvas/<file>.html`
...
```
canvas action:present node:mac-63599bc4-b54d-4392-9048-b97abd58343a target:http://peters-mac-studio-1.sheep-coho.ts.net:18793/__miniclaw__/canvas/snake.html
```
...
1. Check server bind: `cat ~/.miniclaw/miniclaw.json | jq '.gateway.bind'`
2. Check what port canvas is on: `lsof -i :18793`
3. Test URL directly: `curl http://<hostname>:18793/__miniclaw__/canvas/<file>.html`
...
Node is offline. Use `miniclaw nodes list` to find online nodes.
...
The canvas host serves from `/__miniclaw__/canvas/` prefix:

```
http://<host>:18793/__miniclaw__/canvas/index.html  → ~/clawd/canvas/index.html
http://<host>:18793/__miniclaw__/canvas/games/snake.html → ~/clawd/canvas/games/snake.html
```

The `/__miniclaw__/canvas/` prefix is defined by `CANVAS_HOST_PATH` constant.
